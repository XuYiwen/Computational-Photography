...
% Pad to larger images (on both sides of border) to be same height and width
[h1, w1, b1] = size(im1);
[h2, w2, b2] = size(im2);
minw = min(w1, w2);
brdw = (max(w1, w2)-minw)/2;
minh = min(h1, h2);
brdh = (max(h1, h2)-minh)/2;

if minw == w1
    im1 = padarray(im1, [ceil(brdh),ceil(brdw)], 'pre');
    im1 = padarray(im1, [floor(brdh),floor(brdw)], 'post');
else
    im2 = padarray(im2, [ceil(brdh),ceil(brdw)], 'pre');
    im2 = padarray(im2, [floor(brdh),floor(brdw)], 'post');
end

% figure(1), hold off, imshow(im1);
% figure(2), hold off, imshow(im2);